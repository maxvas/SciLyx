	...
	: d(*new GuiViewPrivate(this)), id_(id), closing_(false), busy_(0)
{
    ...
    //MAXVAS: Убрал автосохранение, потому что оно приводило к непонятной ошибке
...
void GuiView::openDocument(string const & fname)
{
    ...
    //MAXVAS: Убрал && !LyXVC::file_not_found_hook(fullname)
...
void GuiView::openOldVersionOfDocument(string const & fname, char *data)
{
    string initpath = lyxrc.document_path;
    ...
    docstring str2;
    TempFile tempfile("oldVersionXXXXXX.lyx");
    tempfile.setAutoRemove(true);
    string outfile = tempfile.name().toFilesystemEncoding();
    ofstream textfile(outfile.c_str());
    textfile<<data;
    textfile.flush();
    textfile.close();
    Buffer * buf = loadDocument(tempfile.name());
    buf->setFileName(fullname);
    if (buf) {
        str2 = bformat(_("Document %1$s opened."), disp_fn);
        if (buf->lyxvc().inUse())
            str2 += " " + from_utf8(buf->lyxvc().versionString()) +
...
"chart", "tabledata", "href", "include", "index", "index_print", "info",
...
Dialog * GuiView::build(string const & name)
{
    ...
    if (name == "tabulardatacreate")
        return createGuiTabularDataCreate(*this);
	if (name == "texinfo")
