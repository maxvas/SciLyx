...
//MAXVAS QApplication ----> QObject
GuiApplication::GuiApplication(int & argc, char ** argv)
    : QObject(0), current_view_(0),
	...
	case LFUN_BUFFER_NEW:
		...
		if (d->views_.empty()
		   || (!lyxrc.open_buffers_in_tabs && current_view_->documentBufferView() != 0)) {
			...
			current_view_->show();
            //MAXVAS: Оставить эту строчку?
            QApplication::setActiveWindow(current_view_);
		} else {
			current_view_->newDocument(to_utf8(cmd.argument()), false);
		}
		break;
...
void GuiApplication::resetGui()
{
	...
	for (it = d->views_.begin(); it != d->views_.end(); ++it) {
		GuiView * gv = *it;
		setCurrentView(gv);
        gv->setLayoutDirection(QApplication::layoutDirection());
		gv->resetDialogs();
	}
...
GuiView *GuiApplication::createView(int view_id)
{
	return createView(QString(), true, view_id);
}


GuiView *GuiApplication::createView(QString const & geometry_arg, bool autoShow,
	int view_id)
{
    ...
    lyx->mainWindowCreatedHandler(view);
    return view;
}
...
//	}
	...
	case QLocale::Urdu :
        //MAXVAS setLayoutDirection------>QApplication::setLayoutDirection
...
void Application::setCloseHandler(ApplicationCloseHandler onCloseHandler)
{
