...
{
	setupUi(this);
    preview = new GraphicsPreview(this);
    PreviewTab->setLayout(new QVBoxLayout());
    PreviewTab->layout()->addWidget(preview);
	...
	//graphics pane
    connect(preview, SIGNAL(changed()),
...
{
    string const bb = readBoundingBox(params_.imageData);
	bbChanged = false;
	if (bb.empty())
		return;
	doubleToWidget(lbX, token(bb, ' ', 0));
...
string GuiGraphics::readBoundingBox(QByteArray const &imageData)
{
    QImage img = QImage::fromData(imageData, "PNG");
    return QString("0 0 %1 %2").arg(img.width()).arg(img.height()).toStdString();
}
...
GraphicsPreview::GraphicsPreview(QWidget *parent)
    :QWidget(parent)
{

}

void GraphicsPreview::setImage(QByteArray data)
{
    image_.loadFromData(data);
}

bool GraphicsPreview::hasImage()
{
    return !image_.isNull();
}

void GraphicsPreview::paintEvent(QPaintEvent *)
{
    if (image_.isNull())
        return;
    QPainter p(this);
    QImage img = image_.scaled(this->width(), this->height(), Qt::KeepAspectRatio);
    p.drawImage((this->width()-img.width())/2, (this->height()-img.height())/2, img);
}
