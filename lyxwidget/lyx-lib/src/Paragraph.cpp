...
Paragraph::Private::Private(Paragraph * owner, Layout const & layout)
    : owner_(owner), inset_owner_(0), id_(-1), begin_of_body_(0), layout_(&layout), readOnly(false)
{
	text_.reserve(100);
}
...
Paragraph::Private::Private(Private const & p, Paragraph * owner)
	: owner_(owner), inset_owner_(p.inset_owner_), fontlist_(p.fontlist_),
      ...
      layout_(p.layout_), readOnly(p.readOnly)
{
	id_ = ++paragraph_id;
...
void Paragraph::setReadOnly(bool readOnly)
{
    d->readOnly = readOnly;
}

bool Paragraph::isReadOnly() const
{
    return d->readOnly;
}
