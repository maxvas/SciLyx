...
Buffer::ReadStatus Buffer::readFile(FileName const & fn)
{
    ...
    //MAXVAS: Я это тупо закомментил, потому что git ls-files не работал
...
bool Buffer::save() const
{
    ...
    TempFile tempfile("editXXXXXX.lyx");
...
//	    (symlink ? savefile.copyTo(fileName(), true) : savefile.moveTo(fileName()))) {
    if (symlink ? savefile.copyTo(fileName(), true) : savefile.copyTo(fileName())) {
