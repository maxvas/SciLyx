...
Inset * createInsetHelper(Buffer * buf, FuncRequest const & cmd)
{
	try {
        ...
        case LFUN_TABULAR_DATA: {
            if (cmd.argument().empty())
                return 0;
            InsetTabular *table = new InsetTabular(buf, 2, 2);
            InsetTabular::string2params(to_utf8(cmd.argument()), *table);
            return table;
        }
		...
		case LFUN_INSET_INSERT: {
			...
			case CHART_CODE: {
				InsetChartParams igp;
				InsetChart::string2params(to_utf8(cmd.argument()), *buf, igp);
				auto_ptr<InsetChart> inset(new InsetChart(buf));
				inset->setParams(igp);
				return inset.release();
			}
...
Inset * readInset(Lexer & lex, Buffer * buf)
{
	...
	if (tmptok == "CommandInset") {
		...
		} else if (tmptok == "Chart") {	
			inset.reset(new InsetChart(buf));
		} else if (tmptok == "Note") {
