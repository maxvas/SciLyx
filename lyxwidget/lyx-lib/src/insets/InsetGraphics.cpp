...
void InsetGraphics::doDispatch(Cursor & cur, FuncRequest & cmd)
{
	...
	case LFUN_GRAPHICS_RELOAD:
        graphic_->update(params_.as_grfxParams());
		break;
...
string InsetGraphics::prepareFile(DocFileName filename, OutputParams const & runparams) const
{
    // The following code depends on non-empty filenames
    if (filename.empty())
        return string();

    string const orig_file = filename.absFileName();
    // this is for dryrun and display purposes, do not use latexFilename
    string const rel_file = filename.relFileName(buffer().temppath());
    ...
    if (runparams.inComment || !filename.isReadableFile())
        return filename.outputFileName(masterBuffer->temppath());
    ...
    boost::tie(status, temp_file) =
            copyToDirIfNeeded(filename, temp_path);
    ...
    string output_file = runparams.nice ?
        filename.outputFileName(masterBuffer->filePath()) :
    ...
    FileName source_file = runparams.nice ? FileName(filename) : temp_file;
    ...
    ErrorList el;
    if (theConverters().convert(&buffer(), temp_file, to_file, filename,
                   from, to, el,
...
{
    string imgFileName = addName(buffer().temppath(), to_utf8(graphic_label)+".png");
    QFile imgFile(imgFileName.c_str());
    if (!imgFile.open(QFile::WriteOnly))
        return;
    imgFile.write(params_.imageData);
    imgFile.close();
    DocFileName filename(imgFileName);
    // If there is no file specified or not existing,
    // just output a message about it in the latex output.
    LYXERR(Debug::GRAPHICS, "insetgraphics::latex: Filename = "
        << filename.absFileName());

    bool const file_exists = !filename.empty()
    ...
    string file_path = prepareFile(filename, runparams);
    latex_str += file_path;
    latex_str += '}' + after;
    // FIXME UNICODE
...
docstring InsetGraphics::xhtml(XHTMLStream & xs, OutputParams const & op) const
{
	...
	if (output_file.empty()) {
		LYXERR0("InsetGraphics::xhtml: Unable to prepare file `" 
                << string() << "' for output. File missing?");
        string const attr = "src='" + string()
...
bool InsetGraphics::setParams(InsetGraphicsParams const & p)
{
    ...
    if (params() == p && p.imageData.size()!=0)
        return false;
...
//	docstring const str = from_utf8(params_.filename.onlyFileName());
    docstring const str = from_utf8("Image");
	...
	for (; it != end; ++it) {
		if (it->lyxCode() == GRAPHICS_CODE) {
			...
			if (params.groupId == inspar.groupId) {
                ...
                params.imageData = inspar.imageData;
