...
void RenderGraphic::update(graphics::Params const & params)
{
    ...
    if (params_.imageData.size()!=0)
    {
        image_->load(params.imageData);
        image_->setPixmap(params);
    }
}
...
//	if (image_ready) {
        dim.wid = image_->width() + 2 * Inset::TEXT_TO_INSET_OFFSET;
        dim.asc = image_->height();
		dim_ = dim;
		return;
//	}
