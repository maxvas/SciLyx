...
void InsetGraphicsParams::browseImageFile()
{

    QString const str = QFileDialog::getOpenFileName(0,
           QObject::tr("Вставка рисунка"), QDir::homePath(), QObject::tr("Рисунки (*.png *.jpg *.bmp)"));
    insertImageFile(str);
}

bool InsetGraphicsParams::insertImageFile(QString fileName)
{
    QFile file(fileName);
...
bool InsetGraphicsParams::insertImageData(QByteArray data)
{
    QImage img;
...
//		os << "\tfilename " << filename.outputFileName(buffer.filePath()) << '\n';
    //MAXVAS: Сохранение картинок внутрь *.lyx
...
//		filename.set(lex.getString(), bufpath);
    if (token == "imagedata") {
        lex.eatLine();
        QByteArray arr(lex.getString().c_str());
