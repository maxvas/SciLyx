...
// Should LFUN_APPENDIX be restricted to top-level paragraphs?
	...
	case LFUN_SET_GRAPHICS_GROUP: {
		...
		if (id.empty())
            ...
            tmp.imageData = inspar.imageData;
			inspar = tmp;
		}
    ...
    case LFUN_TABULAR_DATA:
        ...
        if (doInsertInset(cur, this, cmd, false, true))
            ...
            bv->showDialog("tabulardatacreate");
			...
			FuncStatus & flag) const
{
	...
	case LFUN_DIALOG_SHOW_NEW_INSET:
		if (cmd.argument() == "bibitem")
			...
			code = WRAP_CODE;
        else if (cmd.argument() == "chart")
            code = CHART_CODE;
        else if (cmd.argument() == "tabledata")
            code = TABLEDATA_CODE;
		break;
	...
	case LFUN_PASTE: {
        ...
        if (arg == "png" || arg == "jpeg") {
            enable = theClipboard().hasGraphicsContents();
			break;
		}
