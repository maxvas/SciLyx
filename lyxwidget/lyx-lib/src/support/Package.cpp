...
//					       command_line_system_support_dir);
//	}
    //system_support_dir_ = get_system_support_dir(binary_dir_., command_line_system_support_dir);
    system_support_dir_ = lyx_dir_;

	// Find the LyX executable
	lyx_binary_ = findLyxBinary(abs_binary);

    locale_dir_ = get_locale_dir(system_support_dir_);
    locale_dir_ = FileName(addPath(system_support_dir_.absFileName(), "locale"));
...
//				     command_line_user_support_dir, user_support_dir_);
    user_support_dir_ = FileName(addPath(binary_dir_.absFileName(), "lyx-dir"));
    explicit_user_support_dir_ = false;
