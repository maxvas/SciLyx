			    ...
			    Clipboard::GraphicsType preferedType)
{
    ...
    QString data = theClipboard().getAsGraphics(cur, isFileList).c_str();
    if (isFileList)
    {
        ...
        for (int i=0; i<fileList.size(); i++)
        {
            ...
            if (!params.insertImageFile(file))
            {
                continue;
            }
    ...
    } else
    {
        ...
        InsetGraphicsParams params;
        if (!params.insertImageData(imageData))
            return;
